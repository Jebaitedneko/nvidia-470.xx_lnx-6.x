...
#define NV_DEFINE_SINGLE_NVRM_PROCFS_FILE(name) \
    NV_DEFINE_SINGLE_PROCFS_FILE_READ_ONLY(name, \
...
)
{
    ...
    if (nvlfp->data != NULL)
    {
        os_free_mem(nvlfp->data);
    }

    sp  = nvlfp->fops_sp[NV_FOPS_STACK_INDEX_PROCFS];
    if (sp != NULL)
    {
        nv_kmem_cache_free_stack(sp);
    }
...
nv_procfs_open_file(
    struct inode *inode,
    struct file  *file,
    nv_linux_file_private_t **nvlfpp
)
{
    int retval = 0;
    NV_STATUS status;
    ...
    nvlfp->proc_data = NV_PDE_DATA(inode);
    ...
    status = os_alloc_mem((void **)&nvlfp->data, NV_PROC_WRITE_BUFFER_SIZE);
    if (status != NV_OK)
    {
        retval = -ENOMEM;
        goto done;
    ...
    os_mem_set((void *)nvlfp->data, 0, NV_PROC_WRITE_BUFFER_SIZE);
    nvlfp->fops_sp[NV_FOPS_STACK_INDEX_PROCFS] = sp;

done:
    if (retval < 0)
    {
        nv_procfs_close_file(nvlfp);
        return retval;
    }
...
)
{
    ...
    retval = nv_procfs_open_file(inode, file, &nvlfp);
    if (retval < 0)
    {
        return retval;
    }

    retval = single_open(file, nv_procfs_read_registry, nvlfp);
    if (retval < 0)
    {
        nv_procfs_close_file(nvlfp);
        return retval;
...
)
{
    ...
    nv_linux_file_private_t *nvlfp = s->private;
    nv_state_t *nv;
    nv_linux_state_t *nvl = NULL;
    nvidia_stack_t *sp = nvlfp->fops_sp[NV_FOPS_STACK_INDEX_PROCFS];
...
)
{
    ...
    down(&nvlfp->fops_sp_lock[NV_FOPS_STACK_INDEX_PROCFS]);

    bytes_left = (NV_PROC_WRITE_BUFFER_SIZE - nvlfp->off - 1);
    ...
    proc_buffer = &((char *)nvlfp->data)[nvlfp->off];
    ...
    *pos = nvlfp->off;
    ...
    up(&nvlfp->fops_sp_lock[NV_FOPS_STACK_INDEX_PROCFS]);
...
)
{
    ...
    retval = single_open(file, nv_procfs_show_exercise_error_forwarding, nvlfp);
    if (retval < 0)
    {
        nv_procfs_close_file(nvlfp);
        return retval;
...
)
{
    ...
    nv_linux_file_private_t *nvlfp = s->private;
    nv_state_t *nv = nvlfp->proc_data;
    char *proc_buffer = &((char *)nvlfp->data)[0];
    size_t count = nvlfp->off;
    int i = 0, status = 0;
    NvU32 xid = 0;
...
)
{
    ...
    retval = single_open(file, nv_procfs_read_unbind_lock, nvlfp);
    if (retval < 0)
    {
        nv_procfs_close_file(nvlfp);
        return retval;
...
)
{
    ...
    if (0 != nvlfp->off)
    {
        ...
        if (NULL == nvlfp->data || NULL == nv)
        {
            rc = -EINVAL;
            goto done;
        }

        if (sscanf((char *)nvlfp->data, "%u\n", &value) != 1)
        {
            rc = -EINVAL;
            goto done;
        }
...
)
{
    ...
    nv_linux_file_private_t *nvlfp = s->private;
    nv_stack_t *sp = nvlfp->fops_sp[NV_FOPS_STACK_INDEX_PROCFS];
    nv_state_t *nv = nvlfp->proc_data;
...
)
{
    ...
    retval = single_open(file, nv_procfs_read_offline_pages, nvlfp);
    if (retval < 0)
    {
        nv_procfs_close_file(nvlfp);
        return retval;
...
)
{
    ...
    retval = single_open(file, nv_procfs_read_numa_status, nvlfp);
    if (retval < 0)
    {
        nv_procfs_close_file(nvlfp);
        return retval;
