...
static inline int nvkms_read_lock_pm_lock_interruptible(void)
{
...
#if defined(NV_DEFINE_SINGLE_PROCFS_FILE)
#define NV_DEFINE_SINGLE_NVKMS_PROCFS_FILE(name) \
    NV_DEFINE_SINGLE_PROCFS_FILE(name, \
                                 nvkms_read_lock_pm_lock_interruptible, \
                                 nvkms_read_unlock_pm_lock)
#else
#define NV_DEFINE_SINGLE_NVKMS_PROCFS_FILE(name) \
    NV_DEFINE_PROCFS_SINGLE_FILE(name)
#endif
...
static int nvkms_open(struct inode *inode, struct file *filp)
{
    ...
    status = nvkms_read_lock_pm_lock_interruptible();
    if (status != 0) {
        return status;
    }
